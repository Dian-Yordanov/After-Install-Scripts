/*
Title: Font Library v0.5 (Preview)
...
Group: Compatibility
    ...
    v1.1+ (ANSI and Unicode) (32 and 64 bit).
...
;
;   c{color} - Text color.  "{color}" is a 6-digit hex RGB color value.  Example
;       value: FF00FA.  On input, this option will attempt to pre-select the
;       text color.  On output, this option is returned with the selected text
;       color.  Exception 1: The color Black (000000) is the default text color.
;       On input, Black is pre-selected if this option is not defined.
;       Exception 2: If p_Effects is FALSE, this option is ignored on input
;       and is not returned.
;
...
;       input and is not returned.
;
;   w{font weight} - [Input only] Font weight (thickness or boldness), which is an
;       integer between 1 and 1000 (400 is normal and 700 is bold).  If the font
;       weight is set to a value greater than or equal to 700, this option will
;       pre-select the "bold" font style.
;
...
;   flags may produce unexpected results.  Be sure to test throroughly.
;
;-------------------------------------------------------------------------------
Fnt_ChooseFont(hOwner=0,ByRef r_Name="",ByRef r_Options="",p_Effects=True,p_Flags=0)
    ...
    if p_Flags & CF_EFFECTS
        ...
        r_Options.="c" . SubStr(Fnt_Convert2Hex(l_Color,6),3) . A_Space
        }
...
;
;-------------------------------------------------------------------------------
Fnt_Convert2Hex(p_Integer,p_MinDigits=0)
    {
